#include "dummy_with_tcp_break_imitator.h"
...
{
    // Сбросим номер посылки:
    m_transferNumber = 0;

    // Сбросим счётчик принятых сообщений;
    m_receivedMessageCounter = 0;
...
// В этой функции мы осуществляем привязку вызова функции send() к наступлению таймаута таймера "m_timer".
{
  ...
  // Привязываем вызов функции "messageFrom_DUMMY", к сигналу "modus::IOBuffer::dataReaded". Этот сигнал
  // испускается интерфейсной частью по приходу данных от интерефейса (в нашем случае - от имитатора DUMMY).
  connect(p_io_buffer, &modus::IOBuffer::dataReaded, this, &SvDummyImitator::messageFrom_DUMMY);
...
// Функция, которая вызвается по готовности данных в интерфейсной части:
void apak::SvDummyImitator::messageFrom_DUMMY(modus::BUFF* buffer)
{
    // Команду на разрыв соединения мы будем подавать тогда, когда знаем, что соединение
    // было установлено. Это мы узнаем по тому что мы поличили данные от интерфейса.
    // Чтобы видеть процесс "отсоединенея - присоединения клиента к серверу", будем
    // выдвать команду на разрыв соединения, только после 10 принятых сообщений.

    // Инкрементируем счётчик принятых сообщений;
    m_receivedMessageCounter++;

    if (m_receivedMessageCounter == 10)
    {
        qDebug() << "DUMMY_WITH_TCP_BREAK_CONNECTION: Выдаю команду на разрыв TCP-соединения";
        emit p_io_buffer->say("breakConnection");

        //Сбрасываем счётчик принятых сообщений:
        m_receivedMessageCounter = 0;
    }
}
/** ********** EXPORT ************ **/
