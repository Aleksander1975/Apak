...
// === Имена параметров сигналов КСОН, передаваемых от АПАК к КСОН (эти сигналы описаны ===
// === в файле "to_KSON.json") и от КСОН к АПАК (эти сигналы описаны в файле "data_KSON.json")===
...
// Имя параметра "минимальное значение сигнала". Этот параметр используется для проверки
// допустимости значений сигналов параметрических данных, получаемых нами в информационном
// кадре от КСОН.
#define MIN         "min"
...
// актуальность сигналов каждой из групп, что используется нами при проверки допустимости
// значений сигналов, приходящих от КСОН
#define GROUP       "group"
...
namespace apak
{
    ...
    // - Номер группы, к которой принадлежит сигнал.
    {
        ...
        // или "data_KSON.json" устройства КСОН , и содержащем информацию об одном сигнале.
        ...
        // Выход: Заполненная параметрами сигнала устройства КСОН структура SignalParams.
        {
            ...
            {
                throw SvException(QString("АПАК-КСОН: Ошибка при разборе параметров сигнала: ") + err.errorString());
            }
            try
            {
        ...
        // Выход: Заполненная параметрами сигнала устройства КСОН структура SignalParams.
        {
            ...
            if(object.contains(P))
            {
                if(object.value(P).toInt(-1) < 0)
                    throw SvException(QString("АПАК-КСОН: ") + QString(IMPERMISSIBLE_VALUE)
                               .arg(P)
                ...
                throw SvException(QString("АПАК-КСОН: ") + QString(MISSING_PARAM_DESC).arg(QString(QJsonDocument(object).toJson(QJsonDocument::Compact))).arg(P));
