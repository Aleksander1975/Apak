...
namespace apak {
  ...
  private:
    ...
    // ==== СТРУКТУРЫ ДАННЫХ ДЛЯ ФОРМИРОВАНИЯ ИНФОРМАЦИОННОГО ПАКЕТА ОТ АПАК К КСОН ====:
    ...
    QMap<uint16_t, modus::SvSignal*> m_signal_to_KSON_by_bitNumberInInformBlock;
    ...
    QMap<uint16_t, uint8_t> m_bitNumberInSignal_to_KSON_by_bitNumberInInformBlock;

    // ==== СТРУКТУРЫ ДАННЫХ ДЛЯ ПРОВЕРКИ ИНФОРМАЦИОННОГО ПАКЕТА ОТ КСОН К АПАК ====:
    ...
    QMap<uint8_t, modus::SvSignal*> m_signals_from_KSON_by_byte;
